...
def train_model(train_x, train_y, device, admm_params, input_dim: int=1, backend='nccl'):
    ...
    4. Each agent will train its local model with augmented dataset again. (with or without z consensus update)
    """
    ...
    if rank == 0:
        ...
        print(f"Rank {rank} - Local dataset size: {train_x.size(0)}")
    ...
    def closure_aug():
        ...
        with gpytorch.settings.cholesky_jitter(1e-2), \
            ...
            loss_aug.backward() 
            grad = torch.cat([p.grad.flatten() if p.grad is not None else torch.zeros_like(p).flatten() for p in model_aug.parameters()])
        return loss_aug, grad 
    ...
    for epoch in range(admm_params['num_epochs']):
        ...
        if not torch.isfinite(torch.tensor(closure_aug()[0].item())):
            if rank == 0:
...
if __name__ == "__main__":
    ...
    local_x, local_y = split_agent_data(x_norm, y_norm, world_size, rank, partition='sequential', input_dim=input_dim)
