...
def generate_test_data(num_samples, input_dim: int=1, rank: int=0, world_size: int=1,
    ...
    if input_dim == 1:
        ...
        # test_y = 5 * test_x**2 * torch.sin(12*test_x) + (test_x**3 - 0.5) * torch.sin(3*test_x - 0.5) + 4 * torch.cos(2*test_x) + torch.randn(test_x.size()) * 0.2

        test_y = torch.sin(test_x * (2 * torch.pi)) + torch.randn(test_x.size()) * 0.2

    elif input_dim == 2:
        print("Generating 2D test data is not yet implemented.")
    else:
        raise ValueError("Input dimension must be either 1 or 2 for this function.")

    if rank == 0:
        print("global test_x shape:", test_x.shape)
        print("global test_y shape:", test_y.shape)
